  ...
  render() {
    function Tbody(props){
      ...
      let dataTable=data && data.map((item,index)=>{
        ...
        let role=0;
        let url2=`documentInfo?id=${item.id}`;
        return (
          <tr key={index} style={{background: item.status==0 ? "#f5f7fa" : "#fff"}}>
            ...
            <td>
            {
              item.role==2?null:
              <div>
                <Link to={ url2 } className="none-line"><span className="a-link pointer margin-right-10">预览</span></Link>
    ...
    return (
      <div>
        ...
        <div className="table-data padding-20 background-white" style={{'boxShadow':'rgba(0, 0, 0, 0.25) 0px 0px 1px'}}>
          <table className="table-group panel-5">
              ...
              <div className="table-default">
                <table className="width-max">
                  ...
                  <tr>
                    ...
                    <td><Input className="width-400 padding-top-5 padding-bottom-5" type="textarea" rows={2} placeholder="请输入内容" value={ this.state.form.title }  onChange={this.onChange.bind(this,'title')} /></td>
                  </tr>
                  <tr>
